...
class ReportsAnomaly extends CController {

	private const PROFILE_PREFIX = 'web.morereporting.anomaly.filter.';
	...
	protected function checkInput(): bool {
		$fields = [
			...
			'filter_baseline_days' =>	'string',
			'filter_zscore' =>			'string',
	...
	protected function doAction(): void {
		...
		if ($definition !== null) {
			...
			$filter = [
				...
				'zscore' => $config['zscore_threshold'] ?? '3',
			...
			if ($this->hasInput('filter_set')) {
				...
				CProfile::update(self::PROFILE_PREFIX.'baseline_days',
					$this->getInput('filter_baseline_days', '30'), PROFILE_TYPE_STR
				);
				CProfile::update(self::PROFILE_PREFIX.'zscore', $this->getInput('filter_zscore', '3'),
					PROFILE_TYPE_STR
				);
			...
			elseif ($this->hasInput('filter_rst')) {
				...
				CProfile::delete(self::PROFILE_PREFIX.'baseline_days');
				CProfile::delete(self::PROFILE_PREFIX.'zscore');
			...
			$filter = [
				...
				'baseline_days' => CProfile::get(self::PROFILE_PREFIX.'baseline_days', '30'),
				'zscore' => CProfile::get(self::PROFILE_PREFIX.'zscore', '3'),
		...
		$baseline_days = max(1, (int) $filter['baseline_days']);
		$baseline_time_to = $time_from;
		$baseline_time_from = $time_from - $baseline_days * SEC_PER_DAY;
		...
		$raw_data = $report->getData([
			...
			'time_from' => $time_from,
			'time_to' => $time_to,
			'baseline_time_from' => $baseline_time_from,
			'baseline_time_to' => $baseline_time_to,
			'zscore_threshold' => (float) $filter['zscore']
		]);

		$rows = $report->render($report->compute($raw_data), 'interactive');

		$export_formats = [
			'morereporting.anomaly.json' => 'json',
			'morereporting.anomaly.csv' => 'csv',
			'morereporting.anomaly.yaml' => 'yaml',
			'morereporting.anomaly.pdf' => 'pdf'
		];
		...
		if ($export_format === 'csv' || $export_format === 'pdf') {
			$export_headers = [
				_('Host'), _('Item'), _('Baseline mean'), _('Baseline stddev'), _('Analysis points'),
				_('Anomalies'), _('Anomaly rate %'), _('Max |z|'), _('Worst value'), _('Worst value time')
			];
			$export_rows = array_map(static function(array $row): array {
				return [
					...
					$row['worst_value'] !== null ? round($row['worst_value'], 4) : _('N/A'),
					$row['worst_clock'] !== null ? date('Y-m-d H:i:s', $row['worst_clock']) : _('N/A')
				];
			}, $rows);

			if ($export_format === 'pdf') {
				$html = PdfReportHtml::build(_('Anomaly detection'), $export_headers, $export_rows);
		...
		$response->setTitle(_('Anomaly detection'));

		if ($is_export) {
			$response->setFileName('morereporting_anomaly_'.date('Ymd_His').'.'.$export_format);
		}
