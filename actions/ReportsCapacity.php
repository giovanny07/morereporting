...
class ReportsCapacity extends CController {

	private const PROFILE_PREFIX = 'web.morereporting.capacity.filter.';
	...
	protected function doAction(): void {
		...
		if ($definition !== null) {
			...
			if ($this->hasInput('filter_set')) {
				...
				CProfile::update(self::PROFILE_PREFIX.'threshold', $this->getInput('filter_threshold', '0'),
					PROFILE_TYPE_STR
				);
			...
			elseif ($this->hasInput('filter_rst')) {
				...
				CProfile::delete(self::PROFILE_PREFIX.'threshold');
			...
			$filter = [
				...
				'threshold' => CProfile::get(self::PROFILE_PREFIX.'threshold', '0'),
		...
		$report = new CapacityForecastReport();
		...
		$export_formats = [
			'morereporting.capacity.json' => 'json',
			'morereporting.capacity.csv' => 'csv',
			'morereporting.capacity.yaml' => 'yaml',
			'morereporting.capacity.pdf' => 'pdf'
		];
		...
		if ($export_format === 'csv' || $export_format === 'pdf') {
			$export_headers = [
				_('Host'), _('Item'), _('Current value'), _('Trend (per day)'), _('Days to threshold'),
				_('Estimated date')
			];
			$export_rows = array_map(static function(array $row): array {
				return [
					...
					round($row['slope_per_day'], 4),
					$row['days_to_threshold'] !== null ? round($row['days_to_threshold'], 1) : _('N/A'),
					$row['eta_clock'] !== null ? date('Y-m-d', $row['eta_clock']) : _('N/A')
				];
			}, $rows);

			if ($export_format === 'pdf') {
				$html = PdfReportHtml::build(_('Capacity forecast'), $export_headers, $export_rows);
		...
		$response->setTitle(_('Capacity forecast'));

		if ($is_export) {
			$response->setFileName('morereporting_capacity_'.date('Ymd_His').'.'.$export_format);
		}
